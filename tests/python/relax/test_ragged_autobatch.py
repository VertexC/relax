...
class MatmulExample:
  @R.function
  def main(
    a: R.Tensor(("l", "h"), "float32"),
    b: R.Tensor((h, "k"), "float32"),
  ):
    out = relax.nn.matmul(a, b, "float32")
    return out


mod = MatmulExample
print(MatmulExample.script())
passes = [relax.transform.AutobatchMatmul()]
...
# res = run_cpu(MatmulExample, "main", x, data, w, ind_ptr)
# print(res)
